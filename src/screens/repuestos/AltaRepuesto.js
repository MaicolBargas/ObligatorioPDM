...
const AltaRepuesto = ({ navigation }) => {
  ...
  const altaV = () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        `INSERT INTO repuesto(id, nombre, cantidad) VALUES (?, ?, ?)`,
        [id, nombre, cantidad],
        (tx, results) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }} >
        <View style={styles.mainContainer} >
            ...
            onPress={altaV}>
            <Text style={styles.touchableOpacityTextRegister} > Alta Repuesto </Text>
          </TouchableOpacity>
        </View>
    </SafeAreaView>
  );
